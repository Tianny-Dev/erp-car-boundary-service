...
{
    // --- Core Data Fetching Logic (New reusable method) ---
    ...
    private function fetchRevenueDetails(array $validated): \Illuminate\Support\Collection
    {
        ...
        // NOTE: $validated['payment_date'] should be decoded BEFORE calling parseDateRange
    ...
    // --- Date Parsing Helper (Unchanged, but now receives a clean string) ---
    ...
    private function parseDateRange(string $dateString, string $period): array
{
    ...
    // --- Export Method (Uses new fetcher and decoding) ---
    ...
    public function exportDetails(Request $request)
    {
        ...
        // (This section correctly transforms the Eloquent collection $details into $exportRows)
        $exportRows = collect([]);
        $grandTotals = [
            'amount' => 0.0,
        ...
        if (in_array($exportType, ['excel', 'csv'])) {
            // NOTE: Excel/CSV export needs rows where values are not arrays/objects.
            // We'll format them to two decimal places for the export.
            $excelRows = $exportRows->map(function ($row) use ($breakdownTypesDb) {
                foreach ($row as $key => $value) {
                    ...
                    if (in_array($key, ['total_trip_amount', 'driver_earning']) || in_array($key, $breakdownTypesDb)) {
                        // The number_format returns a STRING.
                        $row[$key] = number_format((float) $value, 2, '.', '');
                    }
                }
                // Only return values (DriverExport requires a plain array of values)
                return array_values($row);
            }); // *** REMOVE ->toArray() HERE ***

            // $excelRows is now an Illuminate\Support\Collection, satisfying the export constructor.
