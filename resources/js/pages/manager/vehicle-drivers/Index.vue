...
const breadcrumbs: BreadcrumbItem[] = [
  { title: 'Assign Drivers', href: manager.vehicleDrivers.index().url },
];
...
const assignDriver = async () => {
  ...
  try {
    await router.put(
      `/manager/vehicle-drivers/${editingVehicle.value.id}`,
...
const removeDriver = async () => {
  ...
  try {
    await router.put(
      `/manager/vehicle-drivers/${confirmVehicle.value.id}`,
      { driver_id: null },
      {
        onSuccess: () =>
