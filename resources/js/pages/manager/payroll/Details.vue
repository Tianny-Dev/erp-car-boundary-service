...
const props = defineProps<{
  ...
  filters: {
    tab: 'franchise' | 'branch'; // Keeping both in tab to support the back link for the parent page
    // franchise: string | null; // <-- REMOVED
    branch: string | null;
    driver_id: string;
...
// --- Export and Go Back functions (UPDATED: Simplified goBack) ---
function handleExport(type: 'pdf' | 'excel' | 'csv') {
  ...
  for (const key in props.filters) {
    ...
    // NOTE: props.filters.franchise is no longer in the prop interface, but iterating over props.filters is safe.
    // If it was still present, we would skip it here: if (key === 'franchise') continue;
...
const goBack = () => {
  const queryParams: Record<string, string> = {
    tab: 'branch', // FORCE to 'branch' since we only support branch filtering now
    period: props.filters.period,
    driver: props.filters.driver_id || 'all',
    service: 'Trips',
  };
  // ONLY check and include 'branch' filter
  if (props.filters.branch) {
    queryParams.branch = props.filters.branch;
  }
  // The franchise property is removed from props.filters, so no need for an else if block.
